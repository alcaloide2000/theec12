    ...
    [
        ...
        dbc.CardBody(
            [
                ...
                dbc.Row(
                    [
                        dbc.Col(html.Label("CHOOSE A SET OF SENTENCES", style={'padding-top': '10px'}), width="auto"),
                        dbc.Col(
                            html.Div(id='dropdownlist', children=[
                                dcc.Dropdown(
                                    id='dropdownlist-dropdown',
                                    options=[],
                                    value=0
                                )
                            ]),
                ...
                html.P('Correct or Incorrect?', className="card-text mt-2"),
                dbc.Button('CORRECT', id='btn-correct', n_clicks=0, color="success", className="me-1"),
                dbc.Button('INCORRECT', id='btn-incorrect', n_clicks=0, color="danger", className="me-1"),
                html.Div(id='correct-counter', className="mt-2"),
                html.Div(id='incorrect-counter', className="mt-2")
            ],
        )
    ],
    style=card_style
...
@app.callback(Output('dropdownlist', 'children'),
              Input('didfthe-stored', 'data')
          ,prevent_initial_call = True)
def populateslider(didfthe):
    if not didfthe:
        return no_update
    dfthe = pd.DataFrame(didfthe)
    listaindices = list(dfthe.index)  # toma los valores únicos de cada indice
    if len(listaindices)<5:
        listaindices =[len(listaindices)]
        index_options = [{'label': str(indice), 'value': indice} for indice in listaindices]
    elif len(listaindices)<10:
        listaindices = [5,len(listaindices)]
        index_options = [{'label': str(indice), 'value': indice} for indice in listaindices]
    elif len(listaindices)<15:
        listaindices = [5,10,len(listaindices)]
        index_options = [{'label': str(indice), 'value': indice} for indice in listaindices]
    elif len(listaindices)<20:
        listaindices = [5,10,15,len(listaindices)]
        index_options = [{'label': str(indice), 'value': indice} for indice in listaindices]
    else:
        listaindices = [5,10,15,20]
        index_options = [{'label': str(indice), 'value': indice} for indice in listaindices]
    return dcc.Dropdown(
        id='dropdownlist-dropdown',
        options=index_options,
        value=listaindices[0]
    )
...
@app.callback(
    ...
    [State("didfthe-stored", 'data'),
     ...
     State('dropdownlist-dropdown','value')],
    prevent_initial_call=True,

)
def display_sentence(btn1, btn2, didfthe, diordenadatoday,number):
    ...
    if button_id == "btn-nclicks-1":
        dfthe = pd.DataFrame(didfthe)
        selected_indices = random.sample(list(dfthe.index), number)
        random_index = random.choice(selected_indices)
        row = dfthe.iloc[[random_index]]
        esp = row.loc[:, 'esp']
        msg = esp
...
@app.callback(
    [Output('correct-counter', 'children'),
     Output('incorrect-counter', 'children')],
    [Input('btn-correct', 'n_clicks'),
     Input('btn-incorrect', 'n_clicks')],
    prevent_initial_call=True
)
def update_counters(btn_correct, btn_incorrect):
    correct_count = btn_correct
    incorrect_count = btn_incorrect

    correct_msg = f"Correct answers: {correct_count}"
    incorrect_msg = f"Incorrect answers: {incorrect_count}"
